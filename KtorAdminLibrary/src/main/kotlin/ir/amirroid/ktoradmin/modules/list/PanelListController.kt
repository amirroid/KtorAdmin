...
private interface PanelHandler {
    ...
    fun calculateMaxPages(totalCount: Long): Long {
...
internal suspend fun ApplicationCall.handlePanelList(
...
private class JdbcPanelHandler(
    private val jdbcTables: List<AdminJdbcTable>,
) : PanelHandler {
...
private class MongoPanelHandler : PanelHandler {
    override suspend fun handleList(
        ...
        val data =
            MongoClientRepository.getAllData(
                panel,
                (currentPage ?: 1) - 1,
    ...
    private fun createSearchFilters(
        ...
        if (searchParameter != null && panel.getSearches().isNotEmpty()) {
...
private suspend fun ApplicationCall.getColumnOrder(table: AdminJdbcTable): Order? {
    ...
    return parameters["order"]?.takeIf { it.isNotEmpty() }?.let { orderColumn ->
        if (orderColumn !in table.getAllColumns().map { it.columnName }) {
            badRequest(
                "The column '$orderColumn' specified in the order does not exist in the table. Please provide a valid column name for ordering.",
            )
        }
...
private suspend fun ApplicationCall.getFieldOrder(table: AdminMongoCollection): Order? {
    ...
    return parameters["order"]?.takeIf { it.isNotEmpty() }?.let { orderField ->
        if (orderField !in table.getAllFields().map { it.fieldName }) {
            badRequest(
                "The column '$orderField' specified in the order does not exist in the collection. Please provide a valid field name for ordering.",
            )
        }
...
private suspend fun buildTemplateModel(
    ...
    return mutableMapOf(
        ...
        "count_text" to
            if (count == 0L) {
                translator.translate(KtorAdminTranslator.Keys.EMPTY)
            } else {
