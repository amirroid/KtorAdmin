package ir.amirroid.ktoradmin.pdf
...
internal object PdfHelper {
        ...
        val rowData =
            JdbcQueriesRepository.getData(this, primaryKey)?.let { items ->
                getAllAllowToShowColumnsInUpsert().mapIndexed { index, column ->
                    ...
                    val url =
                        column.uploadTarget?.let { target ->
    ...
    private fun loadFont(path: String): PdfFont = PdfFontFactory.createFont(path, PdfEncodings.IDENTITY_H)
    ...
    private fun createTable(fontBold: PdfFont): Table {
        val table =
            Table(floatArrayOf(1f, 1f)).apply {
    ...
    private fun createLink(
        ...
        Paragraph(Link(text, PdfAction.createURI(url))).apply {
            setFont(fontRegular)
            setFontSize(12f)
    ...
    private class CheckboxRenderer(
        ...
        override fun draw(drawContext: DrawContext) {
            ...
            if (value == "true") {
                ...
                canvas
                    .moveTo(centerX + 3.5f, centerY + 5f)
