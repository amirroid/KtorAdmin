package ir.amirroid.ktoradmin.processors.mongo
...
import ir.amirroid.ktoradmin.utils.guessFieldPropertyType
import ir.amirroid.ktoradmin.utils.toSuitableStringForFile
...
class MongoCollectionProcessor(
    ...
    private fun generateClass(
        ...
        if (!fieldSets.any { it.fieldName == primaryKey }) {
            throw IllegalArgumentException(
                "(${classDeclaration.simpleName.asString()}) The provided primary key does not match any field in the collection.",
            )
        }
        if (displayFormat != null &&
            ...
            throw IllegalArgumentException(
                "(${classDeclaration.simpleName.asString()}) The provided primary key does not match any field in the cllection.",
            )
        }

        val getAllFieldsFunction =
            FunSpec
        ...
        if (order != null) {
            if (order.name !in fieldSets.map { it.fieldName }) {
                throw IllegalArgumentException(
                    "The field name '${order.name}' specified in the @DefaultOrder annotation does not exist in the column set.",
                )
            }
            if (order.direction.lowercase() !in listOf("asc", "desc")) {
                throw IllegalArgumentException(
                    "The order direction '${order.direction}' specified in the @DefaultOrder annotation must be either 'asc' or 'desc'.",
                )
            }
        }

        val getCollectionNameFunction =
            FunSpec
        ...
        val getCustomActionsFunction =
            FunSpec
                .builder("getCustomActions")
                .addModifiers(KModifier.OVERRIDE)
                .returns(
        ...
        val filterColumns = queryArguments?.findStringList("ir/amirroid/ktoradmin/filtersroid/ktoradmin/filters") ?: emptyList()
        val getSearchColumnsFunction =
            FunSpec
                .builder("getSearches")
                .addModifiers(KModifier.OVERRIDE)
                .returns(
        ...
        val getPluralNameFunction =
            FunSpec
                ...
                .build()
        val getSingularNameFunction =
            FunSpec
                .builder("getSingularName")
                .addModifiers(KModifier.OVERRIDE)
                .returns(String::class)
                .addStatement("return %S", classDeclaration.getSingularName(collectionName))
                .build()
        val getGroupNameFunction =
            FunSpec
        ...
        val getDisplayFormatFunction =
            FunSpec
                ...
                .build()
        val getDatabaseKeyFunction =
            FunSpec
                .builder("getDatabaseKey")
                .addModifiers(KModifier.OVERRIDE)
                .returns(String::class.asTypeName().copy(nullable = true))
                .addStatement("return ${classDeclaration.getDatabaseKey()}")
                .build()
        val getPrimaryKeyFunctions =
            FunSpec
        ...
        val isShowInAdminPanelFunction =
            FunSpec
                        ...
                        if (listItemTypeDeclaration is KSClassDeclaration) {
                            ...
                            if (listFieldType == FieldType.NotAvailable) {
                                ...
                                PropertiesRepository
                                    .getFieldSet(property, FieldType.List(listOf(fieldListSet)))
                                    ?.let { fieldSet ->
                                        properties.add(fieldSet)
                                    }
                            }
                    ...
                    if (mapItemType != null) {
                        ...
                        if (mapItemTypeDeclaration is KSClassDeclaration) {
                            ...
                            if (mapFieldType == FieldType.NotAvailable) {
                                ...
                                PropertiesRepository
                                    .getFieldSet(property, FieldType.Map(listOf(fieldListSet)))
                                    ?.let { fieldSet ->
                                        properties.add(fieldSet)
                                    }
                            }
    ...
    private fun List<KSValueArgument>.findActionList(name: String) =
    ...
    private fun KSClassDeclaration.getDisplayFormat() =
        annotations
    ...
    private fun KSClassDeclaration.getDefaultOrderFormat() =
        annotations
    ...
    private fun KSClassDeclaration.getCollectionName() =
        getAnnotationArguments()
            ?.find { it.name?.asString() == "collectionName" }
            ?.value as? String ?: ""

    private fun KSClassDeclaration.getPrimaryKey() =
        getAnnotationArguments()
    ...
    private fun KSClassDeclaration.getPluralName(collectionName: String) =
        (
            getAnnotationArguments()
                ?.find { it.name?.asString() == "pluralName" }
                ?.value as? String
        )?.takeIf { it.isNotEmpty() } ?: (collectionName + "s")

    private fun KSClassDeclaration.getSingularName(collectionName: String) =
        (
            getAnnotationArguments()
