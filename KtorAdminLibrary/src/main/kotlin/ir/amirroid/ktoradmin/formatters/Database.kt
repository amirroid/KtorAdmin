...
internal fun ResultSet.getTypedValue(
    type: ColumnType,
    name: String,
): Any? =
    when (type) {
...
internal inline fun <reified T> T?.map(columnSet: ColumnSet): T? {
    ...
    if (mappedData != null && mappedData !is T) {
        throw IllegalStateException(
            "ValueMapper '${columnSet.valueMapper}' returned an incompatible type. Expected: ${T::class}, but got: ${mappedData::class}.",
        )
    }
    return mappedData
}
...
internal inline fun <reified T> T?.restore(columnSet: ColumnSet): T? {
    ...
    if (restoredData != null && restoredData !is T) {
        throw IllegalStateException(
            "ValueMapper '${columnSet.valueMapper}' returned an incompatible type. Expected: ${T::class}, but got: ${restoredData::class}.",
        )
    }
    return restoredData
}
