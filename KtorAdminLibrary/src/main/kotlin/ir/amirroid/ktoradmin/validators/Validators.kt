...
import ir.amirroid.ktoradmin.getters.toTypedValueNullable
...
internal object Validators {
    internal fun validateColumnParameter(
        ...
        if (!columnSet.nullable && nullableValue == null) {
            return translator.translate(KtorAdminTranslator.Keys.ERROR_NULL_FIELD)
        }

        // Ensure the field is not blank if 'blank' is set to false
        if (!columnSet.blank && value?.isBlank() == true) {
            return translator.translate(KtorAdminTranslator.Keys.ERROR_EMPTY_FIELD)
        }

        if (columnSet.reference != null) {
            if (value.isNullOrEmpty() && !columnSet.nullable) {
                return translator.translate(KtorAdminTranslator.Keys.ERROR_EMPTY_OR_NULL_REFERENCE)
            }
        }

        if (nullableValue != null && nullableValue.toTypedValueNullable(columnSet.type) == null) {
            return translator.translate(KtorAdminTranslator.Keys.ERROR_INVALID_VALUE)
        }
        ...
        if (columnSet.unique) {
            ...
            if (JdbcQueriesRepository.checkExistSameData(table, columnSet, typedValue, primaryKey)) {
                return translator.translate(KtorAdminTranslator.Keys.ERROR_UNIQUE_FIELD)
            }
        }
...
//        }
        ...
        try {
            ...
            limits?.let {
                ...
                if (it.minDateRelativeToNow != Long.MAX_VALUE && date.isBefore(now.minusDays(minDateInDays))) {
                    ...
                    return translator.translate(KtorAdminTranslator.Keys.ERROR_DATE_BEFORE_MIN, mapOf("date" to minDate.toString()))
                }
                ...
                if (it.maxDateRelativeToNow != Long.MAX_VALUE && date.isAfter(now.plusDays(maxDateInDays))) {
                    ...
                    return translator.translate(KtorAdminTranslator.Keys.ERROR_DATE_AFTER_MAX, mapOf("date" to maxDate.toString()))
                }
            }
        } catch (e: Exception) {
            // Return an error message if the value is not a valid date
            return translator.translate(KtorAdminTranslator.Keys.ERROR_INVALID_DATE)
        }
    ...
    private fun validateDuration(value: String, translator: KtorAdminTranslator): String? {
        try {
            ...
            return translator.translate(KtorAdminTranslator.Keys.ERROR_INVALID_DURATION)
        }
        return null
    }
    ...
    private fun validateDateTime(value: String, limits: Limit?, translator: KtorAdminTranslator): String? {
        try {
            ...
            return translator.translate(KtorAdminTranslator.Keys.ERROR_INVALID_DATETIME, mapOf("format" to Constants.LOCAL_DATETIME_FORMAT))
        }
