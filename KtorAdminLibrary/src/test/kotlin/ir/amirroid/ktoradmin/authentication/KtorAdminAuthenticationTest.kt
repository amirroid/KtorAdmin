...
class KtorAdminAuthenticationTest {
    @Test
    fun `should authenticate valid form credentials and reuse encrypted session`() =
        testApplication {
    ...
    @Test
    fun `should reject invalid form credentials with unauthorized challenge`() =
        testApplication {
            application { configureFormAuthTestApp() }
    ...
    @Test
    fun `should reject form login when csrf token is missing malformed or tampered`() =
        testApplication {
            application { configureFormAuthTestApp() }
            ...
            val tamperedResponse =
                nonRedirectingClient.submitForm(
                    "/admin/login",
                    Parameters.build { append(CSRF_TOKEN_FIELD_NAME, tampered) },
                )
    ...
    @Test
    fun `should reject protected form route when session token is expired`() =
        testApplication {
    ...
    @Test
    fun `should authenticate token flow and reuse encrypted token session`() =
        testApplication {
    ...
    @Test
    fun `should reject token flow for invalid credentials and missing session`() =
        testApplication {
            application { configureTokenAuthTestApp() }
    ...
    @Test
    fun `should enforce role and dashboard permission checks in route authorization`() =
        testApplication {
            application {
                ...
                routing {
                    authenticate("role-auth") {
                        ...
                        get("/editor-only") {
                            ...
                            if (principal.roles?.contains("editor") == true) {
                        ...
                        get("/dashboard") {
                            val principal = call.principal<KtorAdminPrincipal>()!!
                            if (principal.dashboardAccess) {
                                call.respondText("dashboard")
                            } else {
                                call.respondText("no-dashboard", status = HttpStatusCode.Forbidden)
                            }
                        }
                    }
                }
